...
> [!NOTE]
> 公開フォルダはEDCBのHTTPサーバーで公開される場所（デフォルトではHttpPublic配下）です。

> [!WARNING]
> 録画視聴に公開フォルダを選択した場合、当該HTTPアドレスをビデオソースにすると録画ファイルのアクセスに失敗する事があります。  
> 公開フォルダのHTTPアドレスをビデオソースにする場合、録画視聴もビデオソースにする必要があります。
...
| 項目 | 機能 |
----|----
| ドロップログの表示 | 録画情報、及び *RECORD_FILE_NAME.ts.err* ファイルを表示。 |
